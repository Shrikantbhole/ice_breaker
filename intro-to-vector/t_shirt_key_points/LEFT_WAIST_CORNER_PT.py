...
def LEFT_WAIST_CORNER_PT(border_contour, read_image, t_shirt_builder, index):
    reshaped_contour = border_contour.reshape(border_contour.shape[0], 2)

    while True:
        if border_contour[index][1] > border_contour[index + 1][1]\
                        or abs(((border_contour[index][0] - border_contour[index+1][0])/border_contour[index][0]))>Decimal("0.05"):
